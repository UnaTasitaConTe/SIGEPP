...
public class SubjectsController : ControllerBase
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            var result = await _subjectsAppService.GetPagedAsync(query, ct);

            _logger.LogInformation(
                "Se obtuvo página {Page} de asignaturas. Total: {TotalItems}, Retornados: {Count}",
                page,
                result.TotalItems,
                result.Items.Count);
        ...
        {
            _logger.LogError(ex, "Error al obtener lista paginada de asignaturas");
            return StatusCode(
                StatusCodes.Status500InternalServerError,
                new { message = "Error al obtener lista paginada de asignaturas." });
        }
    }
